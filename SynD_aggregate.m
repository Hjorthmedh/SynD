...
function SynD_aggregate()
  ...
  function [columnName, columnData] = aggregateMeanSynapses(dataIdx)
    ...
    for i = 1:length(dataIdx)
      ...
      fracSM(i) = nanmean(data(idx).synapseIntensitySynMean(okMidx) ...
			  ./data(idx).synapseIntensityMorphMean(okMidx));
      fracXM(i) = nanmean(data(idx).synapseIntensityXMean(okMidx) ...
			  ./data(idx).synapseIntensityMorphMean(okMidx));
      fracXS(i) = nanmean(data(idx).synapseIntensityXMean(okSidx) ...
			  ./data(idx).synapseIntensitySynMean(okSidx));
    ...
    for i = 1:length(dataIdx)
      allArea(i)  = nanmean(data(dataIdx(i)).somaArea);
      allMajor(i) = nanmean(data(dataIdx(i)).somaMajorAxisLength);
      allMinor(i) = nanmean(data(dataIdx(i)).somaMinorAxisLength);
    ...
    if(~isempty(data(dataIdx(1)).somaMeasureMorph))
      ...
      for i = 1:length(dataIdx)
	allSomaMorph(i) = nanmean(data(dataIdx(i)).somaMeasureMorph);
	allSomaSyn(i)   = nanmean(data(dataIdx(i)).somaMeasureSyn);
	allSomaX(i)     = nanmean(data(dataIdx(i)).somaMeasureX);
  ...
  function [columnName, columnData] = aggregateSholl(dataIdx, ...
    ...
    columnData{end+1} = nanmean(allData,2);

    columnName{end+1} = 'Std';
    columnData{end+1} = nanstd(allData,[],2);

    columnName{end+1} = 'SEM';
    nOk = sum(~isnan(allData),2);
    columnData{end+1} = nanstd(allData,[],2) ./ sqrt(nOk);
  ...
  function summary = makeSummary(vect)
    ...
    summary(1) = nanmean(vect);
    summary(2) = nanstd(vect);
    summary(3) = nanstd(vect)/sqrt(nnz(~isnan(vect)));
  ...
  function makeBarGraph(plotId, saveName, subplotFlag)
	...
	if(numel(tmp) == 0)
      ...
      dataPoint(barNum) = nanmean(allData{barNum});
      nElem(barNum) = nnz(~isnan(allData{barNum}));
      dataPointSEM(barNum) = nanstd(allData{barNum}) ...
  	                      / sqrt(nElem(barNum));
  ...
  function makeCumHist(plotId, saveName, subplotFlag)
    ...
    for histNum = 1:length(plotGroupID)
      ...
      cumHistData{histNum}  = nanmean(cumsum(allData{histNum},2)...
				      ./ repmat(sum(allData{histNum},2), ...
						1,size(allData{histNum},2)), ...
				      1);
      cumHistSEM{histNum}   = nanstd(cumsum(allData{histNum},2),[],1) ...
	                             ./ sqrt(sum(~isnan(allData{histNum}),1));
  ...
  function makeHist(plotId, saveName, subplotFlag)
	...
	if(isempty(tmp))
      ...
      tmpData = nanmean(allData{histNum},2);
      ...
      histDataSEM(1:length(tmpData),histNum) ...
        = nanstd(allData{histNum},[],2) ./ sqrt(nElem);
  ...
  function makeProfileGraph(plotId,saveName,subplotFlag)
	...
	if(~isempty(tmp))
	  ...
	  = allCenters{profNum}(:,find(memberIdx == maxIdx,1));
        profileData(:,profNum) = nanmean(allData{profNum},2);
        nElem = sum(~isnan(allData{profNum}),2);

        profileDataSEM(:,profNum) = nanstd(allData{profNum},[],2) ...
                                    ./ sqrt(nElem);
      end
  ...
  function makeGradientPlot(plotId, saveName, subplotFlag)
    ...
    for gradNum = 1:length(plotGroupID)
      ...
      gradientData(gradNum,1:maxLen) = nanmean(allData{gradNum},1);
      gradientSEM(gradNum,1:maxLen) = ...
	nanstd(allData{gradNum},[],1) ./ sqrt(sum(~isnan(allData{gradNum})));
